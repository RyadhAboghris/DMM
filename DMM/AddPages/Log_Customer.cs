    ...
    { // DataBase and Tables
        ...
        public void loadDebitData()
        {
            ...
            try
            {
                ...
                gridControl1.DataSource = db.Debit_Customer.Where(x => x.ID_Supplier == id).ToList();
        ...
        private void btn_adddebit_Click(object sender, EventArgs e)
        {
            ...
            DMM.AddPages.Add_DebitCustomer add = new AddPages.Add_DebitCustomer();
            add.id = 0;
        ...
        private void btn_delete_Click(object sender, EventArgs e)
        {
            ...
            if (rs == DialogResult.Yes)
            {
                try
                {
                    ...
                    {
                        db = new DMM.DBDMMEntities();
                        tbAdd = db.Debit_Customer.Where(x => x.ID == id).FirstOrDefault();
        ...
        private void logDebitClearData()
        {
            ...
            try
            {
                id = Convert.ToInt16(txt_id.Text); 
                db = new DBDMMEntities();
                var debit_list = db.Debit_Customer.Select(x => x.ID).ToArray();
            for(int i = 0; i < debit_list.Length; i++)
                {
                    tbAdd = db.Debit_Customer.Where(x => x.ID_Supplier == id).FirstOrDefault();
                    db.Entry(tbAdd).State = EntityState.Deleted;
                    db.SaveChanges();
                }
        ...
        private void logpaymentClearData()
        {
            ...
            try
            {
                id = Convert.ToInt16(txt_id.Text);
                db = new DBDMMEntities();
                var debit_list = db.PaymentCustomers.Select(x => x.ID).ToArray();
                for (int i = 0; i < debit_list.Length; i++)
                {
                    tbPayment = db.PaymentCustomers.Where(x => x.ID_Supplier == id).FirstOrDefault();
                    db.Entry(tbPayment).State = EntityState.Deleted;
                    db.SaveChanges();
                }
